...
<path d="M7.49468 14.3746L16.0632 8.85626C16.185 8.77786 16.3329 8.75103 16.4744 8.78167C16.6159 8.81232 16.7395 8.89793 16.8179 9.01967C16.8963 9.14141 16.9231 9.28931 16.8925 9.43084C16.8618 9.57236 16.7762 9.69592 16.6545 9.77432L8.08593 15.2926C7.96419 15.371 7.81629 15.3979 7.67476 15.3672C7.53324 15.3366 7.40968 15.251 7.33128 15.1292C7.25287 15.0075 7.22604 14.8596 7.25669 14.718C7.28733 14.5765 7.37294 14.453 7.49468 14.3746Z" fill="#4E4E4E"/>
          ...
          <template #popper>
            <div style="
              ...
              <div style="
                ...
                ">
                <div><b>Revenue: </b>Total earnings generated by the pool.</div>
                <div><b>Profits: </b>Net profits after expenses and fees.</div>
                <div>
                  <b>Gas Fees: </b>Transaction costs associated with pool
                  activity.
                </div>
                <div>
                  <b>Number of Trades: </b>Total trades executed within the
                  pool.
                </div>
                <div>
                  <b>TVL: </b>The aggregate value of assets locked in the pool.
                </div>
                <div>
                  <b>Average APR: </b>Average Annual Percentage Rate earned by
                  participants.
                </div>
                <div>
                  <b>Volatility Index: </b>Measure of asset price fluctuations.
                </div>
                <div>
                  <b>Sortino Ratio: </b>Risk-adjusted return evaluation.
                </div>
                <div><b>Sharpe Ratio: </b>Risk-to-reward assessment.</div>
              </div>
            </div>
          </template>
        </VTooltip>
    ...
    <DepositModalFirstStep v-if="networkId > 0 &&
      ...
      tokens.length > 0 &&
      poolActivity
      " :tokenPrices="tokenPrices" :pool="pool" :tokens="tokens" :visibleDepositModal="visibleDepositModal"
      @changeVisibleDeposit="changeVisibleDeposit" />
    <WithdrawModal v-if="networkId > 0 &&
      tokenPrices &&
      pool &&
      tokens.length > 0 &&
      pool.onchain
      " :tokenPrices="tokenPrices" :pool="pool" :tokens="tokens" :visibleWithdrawModal="visibleWithdrawModal"
    ...
    <!-- <CRow class="pool-information-row">
      <div class="pool-information-bar" v-if="pool">
        <div style="width: 12%">
          <div class="pool-information-bar__part__section" v-if="pool.apr">
            <div class="pool-information-bar__part__section__caption-text">
              Pool APR
            </div>
            <div
              class="pool-information-bar__part__section__content-text"
              style="text-shadow: 1px 0 currentColor; letter-spacing: 0.5px"
            >
              {{ pool.apr.total }}%
            </div>
          </div>
        </div>
        <div class="vr" style="color: #00C9FFe5"></div>
        <div
          style="
            width: 85%;
            display: flex;
          ...
          class="px-5"
        >
          <div class="pool-information-bar__part__section">
            <div class="pool-information-bar__part__section__caption-text">
              LP price
            </div>
            <div class="pool-information-bar__part__section__price-text">
              {{ lpInfo.lp }}
            </div>
            <div
              :class="`pool-information-bar__part__section__bottom-chip ${
                lpInfo.percentChange > 0 ? ' positive' : ''
              }`"
            >
              {{ lpInfo.percentChange }}%
            </div>
          </div>
          <div class="pool-information-bar__part__section">
            <div class="pool-information-bar__part__section__caption-text">
            ...
            <div
              ...
              "
              class="pool-information-bar__part__section__price-text"
            >
              ${{ SummarizeTokens(pool?.tokens, cryptoComparePrices) }}
            </div>
            <div v-else class="pool-information-bar__part__section__price-text">
              $ 0
            </div>
            <div
              :class="`pool-information-bar__part__section__bottom-chip ${
                tvlInfo.percentChange > 0 ? ' positive' : ''
              }`"
            >
              {{ tvlInfo.percentChange }}%
            </div>
          </div>
          <div class="pool-information-bar__part__section">
            <div class="pool-information-bar__part__section__caption-text">
            ...
            <div
              :class="`pool-information-bar__part__section__bottom-chip ${
                volume24h.percentChange > 0 ? ' positive' : ''
              }`"
            >
              {{ volume24h.percentChange.toFixed(2) }}%
            </div>
          </div>
          <div class="pool-information-bar__part__section">
            <div class="pool-information-bar__part__section__caption-text">
          ...
          <div class="pool-information-bar__part__section">
            <div
              class="pool-information-bar__part__section__caption-text d-flex align-items-center gap-2"
            >
              Liquidity incentives <img :src="infoIcon" />
            </div>
            <div class="pool-information-bar__part__section__proportion">
              <div class="pool-information-bar__part__section__proportion__top">
                -
              </div>
              <div
                class="pool-information-bar__part__section__proportion__bottom"
              >
                / day
              </div>
            </div>
            <div class="pool-information-bar__part__section__bottom-proportion">
              - / day
            </div>
          </div>
        </div>
    ...
    <!-- <CRow class="pool-information-row">
      <NewTokenDisplay
        ...
        :cryptoComparePrices="cryptoComparePrices"
      />
    </CRow> -->
            ...
            <div class="subsection__item">
              ...
              <div class="subsection__item__content">
                ...
                <div class="subsection__item__content__right">
                  <div class="subsection__item__content__right__top paired">
                    <div class="amount">
                      <CurrencySymbol :symbol="currencySymbol" />{{ formatBigNumber(profitInfo.highest) }}
                    </div>
              ...
              <div class="subsection__item__content">
                ...
                <div class="subsection__item__content__right">
                  <div class="subsection__item__content__right__top paired">
                    <div class="amount">
                      <CurrencySymbol :symbol="currencySymbol" />{{ formatBigNumber(profitInfo.lowest) }}
                    </div>
          ...
          <div class="subsection">
            <div class="subsection__item">
              ...
              <div class="subsection__item__content">
                ...
                <div class="subsection__item__content__right">
                  <div class="subsection__item__content__right__top single">
                    <CurrencySymbol :symbol="currencySymbol" />{{ formatBigNumber(poolVolume, currencyDecimals) }}
                  </div>
                </div>
              </div>
              <div class="subsection__item__content">
                ...
                <div class="subsection__item__content__right">
                  <div class="subsection__item__content__right__top single">
                    <CurrencySymbol :symbol="currencySymbol" />{{ formatBigNumber(poolFees, currencyDecimals) }}
                  </div>
                </div>
              </div>
              <div class="subsection__item__content">
...
// import NewTokenDisplay from '@/components/PoolsDetails/NewTokenDisplay'
// import PoolInfo from '@/components/PoolInfo'
import DepositModalFirstStep from '@/components/modals/DepositModalFirstStep'
...
import Pagination from '@/components/Manage/Pool/Pagination'
...
async function SetNetworkData() {
  ...
  _poolSwapsData = convertSwapsCurrency(_poolSwapsData, currency_prices.value, currency.value)
  ...
  chainPairs.value = _tokenPairs.tokenPairs.filter((t) => {
    let [token0, token1] = t.id.split("-")
    let addresses = pool.value.tokens.map((_t) => _t.address)
    if (addresses.includes(token0) && addresses.includes(token1))
      return true
    return false
  }).map((t) => ({ ...t, Blockchain: DisplayNetwork[chainSelected.value.chain] }))
  let notFlattedTokensData = _tokenPairs.tokens.flat().map((t) => ({ ...t, Blockchain: chainSelected.value.name }))
  poolSwapsData.value = _poolSwapsData
  historicalData.value = _poolHistoricalData
  historicSharesData.value = _poolHistoricalData
  let allPairsData = FormatAllPairsData(_tokenPairs, [..._poolSwapsData], network, currencyDecimals.value)
  let allTokensData = FormatAllTokensData(notFlattedTokensData, [..._poolSwapsData], network, currencyDecimals.value)
  allTokensTableData.value = allTokensData.filter((item) => item['Volume All Time'] > 0)
  allPairsTableData.value = allPairsData.filter((item) => item['Volume All Time'] > 0)
  let formatted_tokens_snapshots = FormatPoolBalances(_historicalBalances)
  let chart_data = FormatTrackingInfoChartWTokens(_poolSwapsData, network, pool.value.tokens)
  ...
  _poolActivity.forEach((item) => {
    ...
    let valueUsd = CalculateJoinExitPrice(null, item, historicalPrices.value, false)
    if (valueUsd)
    ...
    tooltip: {
      ...
      custom({ series, seriesIndex, dataPointIndex, w }) {
        ...
        return (
          ...
          '%' +
          '</div>' +
          '<div style="font-size:clamp(8px, 0.9vw, 14px)">Token Amount: ' + formatBigNumber(balances[seriesIndex]) + ' </div>' +
          '</div>'
        )
      },
    },
