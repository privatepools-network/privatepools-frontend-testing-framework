  ...
  <MainCard>
    ...
    <CRow>
      <div class="d-flex justify-content-between mt-3 flex-wrap">
        <div v-if="width <= 768" @click="onClick" class="button">Compose Pool</div>
      </div>
      <div class="d-flex justify-content-between mt-3 mb-4 flex-wrap">
        <div class="d-flex align-items-center gap-2">
          <svg xmlns="http://www.w3.org/2000/svg" style="cursor: pointer" width="26" height="26" viewBox="0 0 26 26"
            fill="none" :class="{ 'view-mode__active': viewMode == 'cards' }"
          ...
          <svg xmlns="http://www.w3.org/2000/svg" style="cursor: pointer" width="26" height="26" viewBox="0 0 26 26"
            fill="none" :class="{ 'view-mode__active': viewMode == 'rows' }"
          ...
          <div class="d-flex align-items-center gap-2">
            <div style="cursor: pointer;">
              <CFormSwitch size="l" v-model="hidePools" id="hidePools" />
            </div>
        ...
        <div class="manage-pools-filters">
          ...
          <div class="manage-pools-filters__el" id="manage-pools-filters__el">
            ...
            <input placeholder="Search Pools..." class="manage-pools-filters__input" v-model="searchBy" />
          </div>
          <div v-if="width > 768" @click="onClick" class="button">Compose Pool</div>
        </div>
      </div>
    ...
    <CRow>
          ...
          <div class="pools-row__col" v-for="(headCaption, headCaptionIndex) in headers" :key="headCaption">
            ...
            <div class="file-table-header-cell">
              <div class="d-flex align-items-center gap-1" :class="headCaptionIndex !== 0 ? header_cells_inside : ''"
                ...
                <div @click="
                  onDatatableHeaderClick(headCaption)
                  " :class="fontSizeTable">
                  {{ headCaption.toUpperCase() }}
                </div>
                <div v-if="periodsHeaders.includes(headCaption)" style="margin-right: -15px">
                  <div>
      ...
      <div class="pools-cards" v-else-if="viewMode == 'cards'">
        ...
        <PoolCard v-for="(pool, index) in filterByStatus" :key="pool.name" :pool="pool" :index="index"
          @goToPool="goToPool" :inactive="isPoolInactive(pool)" />
      </div>
...
import { FormatAllPoolForTrackingPage, FormatAllToDisplay } from '@/lib/formatter/poolsFormatter'
...
import { DisplayNetwork, Network, networkId, DisplayChain } from "@/composables/useNetwork"
...
const headers = ['Pool Composition', 'Revenue', 'Fees', 'Trades', 'Volume', 'TVL', "APR", "Profit", "Actions"]
const periodsHeaders = ['Revenue', 'Fees', 'Trades', 'Volume', 'TVL', 'APR', "Profit"]
...
async function InitPoolsData(network) {
  return await Promise.all([GetPools(network, null, true, true), GetPoolSwapsData(null, network), GetPoolHistoricValues(null, network), GetHistoricalTvl(network)])
}
...
onMounted(async () => {
  ...
  const networksInfo = await Promise.all(networks.map((network) => InitPoolsData(network)))
  console.log(networksInfo)
  poolsData.value = networks.map((n, index) => networksInfo[index][0]).flat()
  poolSwapsData.value = networks.map((n, index) => networksInfo[index][1]).flat()
  historicValues.value = networks.map((n, index) => networksInfo[index][2]).flat()
  historicTvl.value = networks.map((n, index) => networksInfo[index][3]).flat()
  console.log("HISTORICAL TVL - ", historicTvl.value)
  let _defaultPools = networks.map((n, index) => FormatAllPoolForTrackingPage(networksInfo[index][0], networksInfo[index][1], networksInfo[index][2], networksInfo[index][3], n)).flat()
...
function onDatatableHeaderClick(caption) {
  ...
  if (availableFilterHeaders.map((t) => t.toLowerCase()).includes(caption.toLowerCase())) {
    let sortedAsc = [...pools.value].sort((a, b) => a[caption] - b[caption])
    let sortedDesc = [...pools.value].sort(
      (a, b) => b[caption] - a[caption],
    )

    if (
      JSON.stringify(sortedDesc) == JSON.stringify(pools.value)
    ) {
