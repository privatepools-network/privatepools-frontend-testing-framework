...
const maxBalances = computed(() => {
  ...
  for (let i = 0; i < tokensData.value.length; i++) {
    if (i != leastBalanceIndex.value) {
      let toOptimizeUsdAmount = leastBalanceValue.value / tokensData.value[leastBalanceIndex.value].weight * tokensData.value[i].weight
      result[tokensData.value[i].address] = toOptimizeUsdAmount / tokensData.value[i].price
    }
    else {
      result[tokensData.value[i].address] = leastBalanceValue.value / tokensData.value[i].price
    }
  }
  return result
})
...
function OptimizeValue() {
  ...
  for (let i = 0; i < lineNumbers.value.length; i++) {
    let toOptimizeUsdAmount = usdAmount / tokensData.value[lastDepositChanged.value].weight * tokensData.value[i].weight
