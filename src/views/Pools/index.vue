  ...
  <MainCard>
    ...
    <div class="flex md:flex-row flex-col justify-between mt-3 mb-5 flex-wrap">
      ...
      <div class="flex gap-4 flex-wrap md:flex-row flex-col">
        ...
        <div class="flex items-center gap-4">
          <div class="flex items-center gap-2">
            <SvgIcon v-if="width > 768" :pathData="cardsIconPath"
              :fillColor="selectedView === 'Cards' ? '#00E0FF' : 'white'" @click="selectedView = 'Cards'" />
            <SvgIcon :pathData="tableIconPath" :fillColor="selectedView === 'Table' ? '#00E0FF' : 'white'"
    ...
    <div v-if="selectedView === 'Table'">
      <div v-if="width > 768" class="pools-rows">
              ...
              <div class="d-flex align-items-center gap-1" :class="headCaptionIndex !== 0 ? header_cells_inside : ''"
                ...
                <div v-else-if="headCaption === t('volume')"
                  ...
                  <div class="flex items-center gap-1" @click="
                    ...
                    ">
                    <img :src="filterArrow" :class="ascendFilterBy ===
                        `${t('volume')}_${filterByTimeVolume}` && isReverseSorting
                ...
                <div v-else-if="headCaption === 'APR'"
                  ...
                  <div class="flex items-center gap-1" @click="ascendFilterBy = `APR ${filterByTimeAPR}`; toggleSort()">
                    <img :src="filterArrow" :class="ascendFilterBy === `APR ${filterByTimeAPR}` && isReverseSorting
                ...
                <div v-else :class="'head_caption_text flex items-center gap-1 text-black dark:!text-white'" @click="!headCaption.includes(t('composition')) &&
        ...
        <div v-else-if="user_staked_pools.length === 0 && hidePools"
        ...
        <PoolRow
          ...
          :key="pool.name" :pool="pool" :filters="{ APR: filterByTimeAPR, Volume: filterByTimeVolume }"
          :userPools="user_staked_pools" :inactive="isPoolInactive(pool)" :index="pool?.id"
          @goToPoolWithdraw="goToPoolWithdraw" :rewardsData="rewardsData" @goToPoolCompound="goToPoolCompound"
          @goToCLPool="goToCLPool" @goToPool="goToPool" @goToPoolDeposit="goToPoolDeposit"
          @goToPoolManage="goToPoolManage" @goToCL="goToCL" :isActions="true" />
      ...
      <div v-else>
        ...
        <div class="mobile_table_container">
          <MobileAdvancedTable v-for="(pool) in all_pools
            ...
            .filter((item) => !hideSmallPools || item.TVL > minimalTVL)
            .toSorted((a, b) =>
              Number(isReverseSorting ? b[ascendFilterBy] - a[ascendFilterBy] : a[ascendFilterBy] - b[ascendFilterBy]),
            )" :key="pool.name" :pool="pool" :filters="{ APR: filterByTimeAPR, Volume: filterByTimeVolume }"
            :userPools="user_staked_pools" :inactive="isPoolInactive(pool)" :index="pool?.id"
            @goToPoolWithdraw="goToPoolWithdraw" :rewardsData="rewardsData" @goToPoolCompound="goToPoolCompound"
            @goToCLPool="goToCLPool" @goToPool="goToPool" @goToPoolDeposit="goToPoolDeposit"
    ...
    <div v-else-if="selectedView === 'Cards'">
      <div v-if="all_pools.length === 0 && user_staked_pools === null" class="my-72">
        <LoaderPulse />
      </div>
      ...
      <div class="mt-[5%]">
        <Pagination :perPage="perPage" :pools="all_pools.filter((item) => !hideSmallPools || item.TVL > minimalTVL)"
          :currentPage="currentPage" @changePage="changePage" @changePerPage="changePerPage"
          :perPageOptions="[8, 16, 24]"></Pagination>
      </div>
    </div>
  </MainCard>
</template>
...
import { cardsIconPath, tableIconPath } from '@/assets/SvgIconsPath.js'
...
const selectedView = ref(width.value > 768 ? 'Cards' : 'Table')

const optionsTokens = ref([])
const optionsPoolType = ref([
{ name: 'Index', selected: false },
  { name: 'Pair', selected: false },
])
const optionsPoolAttribute = ref([
  { name: t('new'), selected: false },
  { name: 'Highest TVL', selected: false },
  { name: 'Highest APR', selected: false },
])

const perPage = ref(8)
const currentPage = ref(1)

watch(() => hideSmallPools.value, (newValue) => {
  currentPage.value = 1
})
...
function filterTwoTokens(arrayOfObjects, more) {
  return arrayOfObjects.filter(obj => {
    const tokens = obj.tokens || [];
    return more ? tokens.length > 2 : tokens.length === 2;
  });
}

function filterByTimeCreated(pools, days) {
  const now = Date.now() / 1000;
  return pools.filter(item => item.time_created >= now - days * 60 * 60 * 24);
}

function filterByUserStakedPools(pools, userStakedPoolIds) {
  return pools.filter(item => userStakedPoolIds.includes(item.id));
}

function changeAscend(value) {
  ascendFilterBy.value = value
}

const all_pools = computed(() => {
  let result = [...pools.value];

  if (optionsPoolType.value[0].selected && !optionsPoolType.value[1].selected) {
    result = filterTwoTokens(result, true);
  }
  if (optionsPoolType.value[1].selected && !optionsPoolType.value[0].selected) {
    result = filterTwoTokens(result, false);
  }

  if (hidePools.value) {
    const user_pools_ids = user_staked_pools.value.map((item) => item.id);
    result = filterByUserStakedPools(result, user_pools_ids);
  }

  if (optionsPoolAttribute.value[0].selected) {
    result = filterByTimeCreated(result, 30);
  }

  if (optionsPoolAttribute.value[1].selected) {
    changeAscend('TVL')
  }

  if (optionsPoolAttribute.value[2].selected) {
    changeAscend('APR 7D')
  }
