<template>
  <div class="chain_selector_container">
    ...
    <div
      v-for="(chain, i) in excludeAll
        ? chainOptions.filter(function (el) {
            return el.name != 'All Chains'
          })
        : chainOptions"
      :key="`${i}-chain`"
    >
      <div
        :class="
          chainSelected.name === chain.name ? 'chain_tag__active' : 'chain_tag'
        "
        @click="() => updateChain(chain)"
      >
        {{ console.log('chain.name', chain.name) }}
        <img :src="chain.img" class="chain_image" v-if="chain.img !== ''" />
        <span
          v-if="width > 768 || chain.name === 'All Chains'"
          class="chain_text"
          >{{ chain.name }}</span
        >
      </div>
...
const chainSelected = ref(
  path.value === '/track/opportunities' ||
    path.value === '/admin/bot' ||
    path.value == '/rewards'
    ? chainOptions[0].name == 'All Chains'
      ? chainOptions[1]
      : chainOptions[0]
    : chainOptions[0],
)
