...
export function AnalyzeDrawdowns(
  ...
  let allTvlProfitsSum = chart_data.map((item) => ({
    ...
    value:
      parseFloat(item[`TVL${postfix}`][chainSelected]) +
      parseFloat(item[`Profits${postfix}`]),
  }))
  ...
  let troughIndex = tvlProfitSums.findIndex(
    (item) =>
      item.timestamp ==
      (afterPeakValues.length > 0 ? afterPeakValues[0].timestamp : 0),
  )
  ...
  return [
    {
      ...
      value: `${symbol}${formatBigNumber(maximum_drawdown, 5)}`,
    },
    {
      text: 'Duration',
      value: `${Math.floor(
