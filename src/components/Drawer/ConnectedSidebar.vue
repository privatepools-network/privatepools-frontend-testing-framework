  ...
  <div style="height: 92%; padding-top: 7px">
    ...
    <div class="d-flex flex-column h-100 gap-3 mt-3">
      <div class="d-flex flex-column">
        <div class="balance_text text-black dark:!text-white" v-if="sidebarData?.userBalance?.total">
      ...
      <div v-if="sidebarTab === 'Tokens'" class="d-flex flex-column gap-2">
        <div
          ...
          v-for="(item, i) in sidebarData?.userBalance?.tokens.filter((item) => item.usdAmount !== 0 || item.symbol === 'PPN')"
          :key="`${i}-token`"
...
const computedAddress = computed(
  () =>
    props.address ? props.address.substring(0, 6) + '....' + props.address.substring(props.address.length - 4) : '',
)
...
onMounted(async () => {
  ...
  sidebarData.value = await getSidebarData(props.address);
})
