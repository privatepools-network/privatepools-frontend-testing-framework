<template>
  <CTable borderless hover :class="{'table_full':isFullTable==true, 'table_small':isFullTable!=true, 'table_small_loading':isFullTable!=true && displayTable == null}">
                ...
                <div>
                  ...
                  filtersSelected[headCaptionIndex] = $event
                    " placeholder="" openDirection="bottom" label="title" track-by="code"
                               ...
                               :searchable="false" :allow-empty="false" :close-on-select="true" :multiple="false"
                               :taggable="true">
    ...
    <CTableBody class="file-table-body" style="position:relative;">
      ...
      <CTableRow v-if="displayTable && displayTable.length > 0 && !displayTable[0]['VS USD'] && !isFullTable" class="file-table-row" :class="table_bg && rowHeight ? rowHeight : 'file-table-row'"
...
.table_full {
  ...
  .file-table-header-container_full, .file-table-cell-container {
    width: 100px;
    @media (max-width: $xxl) {
      width: 50px;
    }
...
.table_small {
  .file-table-header-container, .file-table-cell-container {
