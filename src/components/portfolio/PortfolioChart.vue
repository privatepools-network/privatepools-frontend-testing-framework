<template>
  <div class="pools_chart_container">
    ...
    <div class="track_chart_card">
      ...
      <div v-if="allChartData.length == 0" class="chart_inside">
        <LoaderPulse />
      </div>
...
const dataVolume = computed(() => {
  if (
    preFiltersList.value.find((f) => f.code == 'Volume').selected
  )
...
function InitChartData() {
  ...
  let chart_data = networks.map((n, i) => isRightChainName(DisplayNetwork[n], chainSelected.value) && networks_data.value[i] ? FormatTrackingInfoChart(networks_data.value[i][1], n) : []).flat()
  if (chart_data.length == 0) {
    allChartData.value = []
    return []
  }
  let historical_tvl = networks.map((n, i) => isRightChainName(DisplayNetwork[n], chainSelected.value) && networks_data.value[i][3] ? networks_data.value[i][3] : []).flat()
  let tokenSnapshots = networks.map((n, i) => (isRightChainName(DisplayNetwork[n], chainSelected.value) && networks_data.value[i][4] ? networks_data.value[i][4].tokenSnapshots : [])).flat()
  let tokensData = networks.map((n, i) => (isRightChainName(DisplayNetwork[n], chainSelected.value) && networks_data.value[i][4] ? networks_data.value[i][4].tokens : [])).flat()
  historical_tvl = historical_tvl.filter(v => v != null)
  let formatted_tvl = FormatHistoricalTvl(historical_tvl)
  let formatted_token_snapshots = FormatTokenSnapshots(tokenSnapshots, tokensData)
...
function formatChartData(formatted_tvl, formatted_token_snapshots, chart_data) {
  ...
  for (let i = 0; i < chart_data.length; i++) {
    ...
    let index = formatted_token_snapshots.Dates.findIndex(s => s == chart_data[i].Date)
    index = index == -1 ? lastIndex : index
    lastIndex = index
    for (let k = 0; k < tokens.length; k++) {
      chart_data[i].Assets[tokens[k]] = formatted_token_snapshots[tokens[k]][index]
    }
