        ...
        >
          <div class="d-flex align-items-center gap-2">
            ...
            >
              ...
              <div class="stats_text" style="color: rgba(1, 180, 126, 1)">
                USD
              </div>
            </div>
          </div>
...
const props = defineProps(['chainSelected', 'poolSwapsData', 'tokensData', 'historical_tvl', 'chartData', 'historicalPrices', 'userFirstTimestamp', 'tokenPairs'])
const { chainSelected, poolSwapsData, tokensData, historical_tvl, chartData, historicalPrices, userFirstTimestamp, tokenPairs } = toRefs(props)
            ...
            tooltip: {
                ...
                custom({ series, seriesIndex, dataPointIndex, w }) {
                    return (
                        ...
                        '<span>' +
                        '<span style="font-weight:700">' + 'TVL:  :' + '</span>' + ' ' + `$${formatBigNumber(w.config.data[seriesIndex].invested)}` +
                        '</span>' +
                        '<span>' +
                        '<span style="font-weight:700">' + 'Allocation:' + '</span>' + ' ' + `${formatBigNumber(w.config.data[seriesIndex].percent)}%` +
                        '</span>' +
                        '<span>' +
                        '<span style="font-weight:700">' + 'Nº Trades:' + '</span>' + ' ' + w.config.data[seriesIndex].trades +
                        '</span>' +
        ...
        return [
            {
                ...
                value: `$${formatBigNumber(getAverageProfitPerTimeRange(poolSwapsData.value, chainSelected.value.name, ...datePickerProfit.value))}`,
            },
            {
                text: 'Average Profit per Pool',
                value: `$${formatBigNumber(getAveragePoolProfitPerTimeRange(poolSwapsData.value, chainSelected.value.name, historical_tvl.value, ...datePickerProfit.value))}`,
            }
        ]
    } else {
        return []
...
const drawDownData = computed(() => chartData.value.length > 0 ? AnalyzeDrawdowns(chartData.value, chainSelected.value.name, ...datePickerDrawdown.value) : [])
