    ...
    <div class="d-flex gap-4">
      <div class="overview_big_container bg-[#FFFFFF24] dark:!bg-[#22222224]" v-for="(item, i) in bigContainerMock"
        ...
        <div>
          ...
          <div class="text_value text-black dark:!text-white">{{ item.name.includes('APR') ? `${parseFloat(item.value).toFixed(3)}%` : parseFloat(item.value).toFixed(3) }}</div>
        </div>
        <div>
          <div class="text_header dark:!text-[#ffffffb2] text-[#000000B2]">{{ item.description }}</div>
          <div class="text_value text-black dark:!text-white">{{ item.name.includes('APR') ? `${parseFloat(item.value_2).toFixed(3)}%` : parseFloat(item.value_2).toFixed(3) }}</div>
        </div>
        <div class="mt-5">
