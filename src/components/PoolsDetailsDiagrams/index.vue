      ...
      <CCol xl="4">
        ...
        ">
            ...
            <div class="d-flex gap-3">
              <div style="cursor: pointer; font-size: clamp(10px, 0.8vw, 14px)" :style="{
                color:
                  opt == profitRangeOption ? '#00C9FF' : 'white',
...
const chartOptions0 = computed(() => {
  return {
    ...
    tooltip: {
      ...
      y: {
        formatter: (val, opt) => {
          return `${props.symbol}${val} (${assetsPerformanceData.value[`${assetsPerformanceRangeOption.value}_tokens`][opt.seriesIndex].data[opt.dataPointIndex]} <img src="${getTokenEntity(assetsPerformanceData.value[`${assetsPerformanceRangeOption.value}_tokens`][opt.seriesIndex].name, 'short').icon}" width="15"> )`
        }
      }
    },
    grid: {
