<template>
  <CContainer class="diagrams-container">
    <CRow class="diagram-row">
      <CCol xl="4">
        <div v-if="assetsPerformanceData[assetsPerformanceRangeOption] &&
        ...
        width: 400px;
        ">
                    <h6 style="font-size:clamp(10px, 0.9vw, 16px)">Information</h6>
                    <div style="display: flex; flex-direction: column; font-size: clamp(10px, 0.8vw, 14px);">
                      <div>This chart offers a historical perspective on the values of assets deposited in the pool. It
                        allows you to track the performance and fluctuations of the assets over time.</div>
                    </div>
                  </div>
                </template>
              </VTooltip>
            ...
            <div class="d-flex gap-2">
              <div style="cursor: pointer; font-size: clamp(10px, 0.8vw, 14px)" :style="{
                color:
      ...
      <CCol xl="4">
        <div v-if="tradesData[tradeRangeOption] && tradesTimestamps[tradeRangeOption]
          " class="d-flex flex-column gap-2 chart_card">
          <div class="d-flex justify-content-between text-white">
            <div class="d-flex align-items-center gap-2">
        ...
        ">
                    <h6 style="font-size:clamp(10px, 0.9vw, 16px)">Information</h6>
                    <div style="display: flex; flex-direction: column; font-size: clamp(10px, 0.8vw, 14px);">
                      <div>This bar chart visually presents the number of trades executed by this pool during a specific
                        time frame. It offers a clear and concise overview of the pool's trading activity. </div>
                    </div>
                  </div>
                </template>
              </VTooltip>
                ...
                <div style="cursor: pointer" @click="tradesType = 'curve'">
                  <svg width="15" height="15" viewBox="0 0 28 26" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path
                      ...
                      :fill="tradesType == 'curve' ? '#01B47E' : 'white'" />
                  </svg>
                </div>
                <div style="cursor: pointer" @click="tradesType = 'bar'">
                  <svg width="15" height="15" viewBox="0 0 29 26" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path
                      ...
                      :fill="tradesType == 'bar' ? '#01B47E' : 'white'" />
                  </svg>
                </div>
              </div>
            </div>
      ...
      <CCol xl="4">
        <div v-if="profitsData[profitRangeOption] &&
        ...
        ">
                    <h6 style="font-size:clamp(10px, 0.9vw, 16px)">Information</h6>
                    <div style="display: flex; flex-direction: column; font-size: clamp(10px, 0.8vw, 14px);">
                      <div>This bar chart visually represents the profit generated by this pool over a specified period.
                        It provides a clear and intuitive way to understand the pool's financial performance.</div>
                    </div>
                  </div>
                </template>
              </VTooltip>
            ...
            <div class="d-flex gap-3">
              <div style="cursor: pointer; font-size: clamp(10px, 0.8vw, 14px)" :style="{
                color:
                  opt == profitRangeOption ? 'rgba(1, 180, 126, 1)' : 'white',
...
const { tradesData, tradesTimestamps, profitsTimestamps, profitsData, assetsPerformanceData, assetsPerformanceTimestamps, tokens } = toRefs(props)
  ...
  return {
    ...
    legend: {
      ...
      tooltipHoverFormatter: function (val, opts) {
        return (
          val + ` ${assetsPerformanceData.value[`${assetsPerformanceRangeOption.value}_tokens`][opts.seriesIndex].data[opts.dataPointIndex]} ` +
          ' - ' +
    ...
    tooltip: {
      ...
      y: {
        formatter: (val, opt) => {
          return `${props.symbol}${val} (${assetsPerformanceData.value[`${assetsPerformanceRangeOption.value}_tokens`][opt.seriesIndex].data[opt.dataPointIndex]} ${assetsPerformanceData.value[`${assetsPerformanceRangeOption.value}_tokens`][opt.seriesIndex].name})`
        }
      }
    },
    grid: {
  ...
  return {
    title: {
      text: `${props.symbol}${parseFloat(
        profitsData.value[profitRangeOption.value].reduce((sum, v) => sum + parseFloat(v), 0),
    ...
    tooltip: {
      ...
      y: {
        formatter: (val, opt) => {
          let items = profitsData.value[`${profitRangeOption.value}_tokens`][opt.dataPointIndex]
          let result_str = ''
          console.log(items)
          let results = {}
          for (let i = 0; i < items.length; i++) {
            let item = items[i]
            let symbol = tokens.value.find((t) => t.address == item.token.toLowerCase()).symbol
            if (!results[symbol])
              results[symbol] = 0
            results[symbol] += item.value
          }
          for (const [key, value] of Object.entries(results)) {
            result_str += `${key} <img src=${getTokenEntity(key, 'short').icon} width="10"/> ${formatBigNumber(value, 3)} <br>`
          }
...
//   () => props.historicPercentagesData[percentageRangeOption.value],
// )
...
//             },
//           ],
//           backgroundBarColors: ['rgba(229, 229, 239, 1)'],
...
//     xaxis: {
//       categories:
//         props.historicPercentagesTimestamps[percentageRangeOption.value],
//       labels: {
//         hideOverlappingLabels: true,
//         rotate: -65,
//         style: {
...
// const series5 = computed(() => props.volatilityData)
...
//         show: false,
//       },
//       categories: props.volatilityTimestamps,
//     },
//     tooltip: {
//       theme: false,
//       y: [
...
.chart_card {
  ...
  border-radius: 20px;
  /* background-color: rgba(23, 31, 31, 0.541); */

}

:deep(.apexcharts-tooltip) {
  background: linear-gradient(rgba(89, 89, 89, 1),
      rgba(73, 73, 73, 0.45)) !important;
  color: white;
