<template>
  ...
  <div class="stats_container">
    ...
    <div class="stats_column gap-xxl-5 gap-2">
    ...
    tooltip: {
      ...
      custom({ series, seriesIndex, dataPointIndex, w }) {
        return (
          ...
          '</span>' +
          ' ' +
          `${symbol}${formatBigNumber(w.config.data[seriesIndex].invested)}` +
          '</span>' +
          '<span>' +
          '<span style="font-weight:700">' +
          'Allocation:' +
...
const profitsData = computed(() => [
  {
    ...
    value: `${symbol.value}${formatBigNumber(
      getAverageProfitPerTimeRange(
        poolSwapsData.value,
        chainSelected.value.name,
...
async function InitApr() {
  if (chartData.value.length > 0) {
    let [aave, compound, celsius, lido, yearn] = await Promise.all([
      GetAaveAprs(),
      GetCompoundAprs(),
      GetCelsiusAprs(),
      GetLidoApr(),
...
watch(chainSelected, async () => {
  if (chartData.value.length > 0) {
    ...
    investmentsData.value = await FormatRoiStatistics(
      chartData.value,
      allTokensInfo.value,
      top10Tokens.value,
      chainSelected.value.name,
